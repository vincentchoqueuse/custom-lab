...
// whose harmonics fold one by one) sampled at fe, reconstructed by ideal
// low-pass (sinc) interpolation over the band [−fe/2, fe/2].
//   folded frequency: r = f mod fe, f_app = min(r, fe − r)
...
/** Fold a frequency into the first Nyquist zone [0, fe/2]. */
function fold(f, fe) {
  const r = ((f % fe) + fe) % fe;
  return Math.min(r, fe - r);
}
 ...
 */
export function compute({ source, f, fe }) {
  ...
  // samples at fe
  const n = Math.max(2, Math.floor(T * fe) + 1);
  ...
  // ideal low-pass reconstruction from the samples (band [−fe/2, fe/2])
  const rx = new Float64Array(NG);
  for (let i = 0; i < NG; i++) {
    let acc = 0;
    for (let k = 0; k < n; k++) acc += sx[k] * sinc(fe * (gt[i] - st[k]));
    rx[i] = acc;
  }

  // line spectra: true harmonics, and their folded images in [0, fe/2]
  const lines = harmonics(source, f);
  const specF = Float64Array.from(lines, (l) => l.f);
  const specA = Float64Array.from(lines, (l) => l.a);
  const aliasF = Float64Array.from(lines, (l) => fold(l.f, fe));
  const fApp = fold(f, fe);

  // folding diagram f_app(f) at the current fe, plus the no-aliasing diagonal
  const NF = 241;
