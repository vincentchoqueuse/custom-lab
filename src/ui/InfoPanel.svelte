<script>
  ...
  //
  // THREE THINGS, and only the first is optional:
  //   · the description, in prose, when the manifest carries one — written
  //     for any reader, distilling what the scenes demonstrate. It absorbed
  //     the old per-scene teacher notes: prose addressed to oneself became
  //     prose addressed to whoever opens the panel, and the private channel
  //     was retired with nothing left to carry;
  ...
  import { app, manifest, applyPreset } from '../core/store.svelte.js';
  ...
  // PROSE FLOWS. A `doc` is written in a template literal and therefore
  // carries the source file's own wrapping at some eighty columns; rendered
  // as-is it breaks mid-sentence at a width that has nothing to do with the
  // panel's. Split on blank lines, and each paragraph wraps to the reader's
  // width.
